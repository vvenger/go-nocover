...
func Run(coveragePath, outputPath, root string, cfg *config.Config) error {
	...
	profile, err := process(coveragePath, root, opts, cfg.MarkNoCoverValue(), cfg.MarkOptionsValue())
	if err != nil {
...
func process(coveragePath string, root string, opts []ast.ExcludeFunc, markNoCover, markOptions config.MarkBlocks) (*parser.Profile, error) {
	...
	profile.Blocks = filter(profile.Blocks, excludeRanges, markNoCover, markOptions)
